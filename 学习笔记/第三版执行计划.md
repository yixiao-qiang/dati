...
- 验收（**2026-09-16 按 g++ 13.3.0 / glibc 2.39 / 内核 7.0 实测修正**）：得到 g++ 实际使用的系统调用列表，断言分两层：
  - **glibc 启动期必现 5 项**（实测全在）：`rseq`、`set_robust_list`、`mprotect`、`arch_prctl`、`getrandom`；
  - **syscall 族覆盖**（族内至少实测到一个，具体成员随 gcc/glibc 版本变化）：进程创建族 `clone3/clone/vfork`、文件状态族 `statx/newfstatat/fstat`、内存映射族 `mmap/mprotect/munmap/mremap`。
- **实测偏差（相对 g++ 11.4 / glibc 2.35 的旧预期）**：进程创建实际命中 `vfork`（8 次，g++ driver 以 `vfork`+`execve` 派生 cc1plus/as/collect2），**无 `clone3`**；文件状态实际命中 `newfstatat`（206 次）、`fstat`（387 次），**无 `statx`**；`madvise` 未出现（malloc 的可选 hint，最小编译负载不触发；脚本保留为 INFO 项不计失败，待大编译负载复测）。
...
| 0.4 | g++ syscall 序列 | 5 个 glibc 必现项 + 3 个 syscall 族覆盖（实测进程创建命中 `vfork`、文件状态命中 `newfstatat/fstat`），产物落盘 |
...
- **把特定版本的 syscall 当硬编码真理**：任务 0.4 旧清单按 g++ 11.4/glibc 2.35 预期 `clone3/statx/madvise`，g++ 13.3/glibc 2.39 实测为 `vfork/newfstatat` 且无 `madvise`。断言要写“syscall 族覆盖”，白名单只认 trace 实测值。
...
//   必须包含（以任务 0.4 实测为准；g++ 13.3 / glibc 2.39）：
//             vfork, execve, newfstatat, fstat, rseq, set_robust_list,
//             mprotect, arch_prctl, getrandom（clone3/statx/madvise 未实测到，暂不放行）
