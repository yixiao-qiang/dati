...
| 方案 | 前端跑在哪 | 你怎么看 | 代价 |
|---|---|---|---|
| **A（推荐）** | VM 内 | Windows 浏览器打开 `http://192.168.40.128:5173` | VM 需装 `nodejs`/`npm`；dev server 必须 `--host 0.0.0.0` |
| B | Windows 内 | 浏览器打开 `http://localhost:5173` | node 已就绪（v24.18.0）；但需在 `vite.config.ts` 配 proxy 指向 VM 的 API |

**为什么推荐 A**：前后端同机，API 直连 `localhost:8080`，不用处理跨域；且贴合"整个系统在 VM 内可复现"的目标。方案 B 是本机 node 现成的省事路线，代价是多一层跨机代理配置。

**三个必知细节**：

1. **Vite 默认只监听 `127.0.0.1`** —— 必须 `npm run dev -- --host`，否则 VM 外访问不到。这是"页面打不开"的头号原因。
...
# 然后在 Windows 浏览器打开：http://192.168.40.128:8000
# 看到目录列表 = 宿主→VM 的端口链路通
```

> 本机 VM 为 NAT 模式（IP `192.168.40.128`），宿主已实测可 ping 通该地址（0% 丢包），端口访问走同一链路。
