...
flowchart TD
    ...
    B --> C["依赖库<br/>libseccomp / libcap / libargon2 / libhiredis / libssl / nlohmann-json"]
    C --> D["Redis 7 + MySQL 8"]
...
sudo apt install -y build-essential cmake ninja-build git curl pkg-config strace gdb
```
...
ninja --version
strace -V          # ★ Wave 0 任务 0.4 的核心工具，缺它做不了白名单生成
```
...
```bash
sudo apt install -y libseccomp-dev libcap-dev libargon2-dev libhiredis-dev libssl-dev nlohmann-json3-dev
```

核对：

```bash
dpkg -l | grep -E 'libseccomp-dev|libcap-dev|libargon2-dev|libhiredis-dev|libssl-dev|nlohmann-json3-dev'
```

> - `libseccomp-dev` — Wave 0 任务 0.3 / Wave 1 任务 1.3（seccomp 白名单）
> - `libcap-dev` — Wave 1 能力管理
> - `libargon2-dev` — Wave 3 任务 3.1（口令哈希）
> - `libhiredis-dev` + `libssl-dev` — redis-plus-plus 的底座依赖
> - `nlohmann-json3-dev` — Wave 3 任务 3.4（`json::parse` 解析请求体）
...
> **结果：12/12 全绿**（VM 内实机输出，逐项核销）。

| # | 检查项 | 命令 | 期望 | 实测 | 判定 |
|---|---|---|---|---|---|
| E1 | 系统版本 | `lsb_release -a` | 24.04.3 | Ubuntu 24.04.3 LTS (noble) | ✅ |
| E2 | 编译器 | `g++ --version` | 可用 | **g++ 13.3.0** | ✅ |
| E3 | CMake | `cmake --version` | ≥ 3.22 | **3.28.3** | ✅ |
| E4 | 依赖库 | `dpkg -l \| grep -E ...` | 6 项齐全 | 见下 | ✅ |
| E5 | Redis | `redis-cli XAUTOCLAIM --help` | 无 unknown | **7.0.15**，`XAUTOCLAIM OK` | ✅ |
| E6 | MySQL | `mysql --version` | 8.0.x | **8.0.46** | ✅ |
| E7 | redis-plus-plus | `ls /usr/local/lib/libredis++*` | 存在 | `.a` / `.so` / `.so.1` / `.so.1.3.15` | ✅ |
| E8 | cgroup v2 | `stat -fc %T /sys/fs/cgroup` | cgroup2fs | `cgroup2fs` | ✅ |
| E9 | unshare | `sudo unshare ... /bin/true` | 退出码 0 | **`exit=0`** | ✅ |
| E10 | LSM | `cat /sys/kernel/security/lsm` | 记录 | `lockdown,capability,landlock,yama,apparmor,ima,evm` | ✅ |
| E11 | strace | `strace -V` | 可用 | **6.8** | ✅ |
| E12 | JSON 库 | `ls /usr/include/nlohmann/json.hpp` | 存在 | 存在（nlohmann-json3-dev 3.11.3） | ✅ |

**E4 明细**：`libseccomp-dev 2.5.5`、`libcap-dev 2.66`、`libargon2-dev 20190702`、`libhiredis-dev 1.2.0`、`libssl-dev 3.0.13`、`nlohmann-json3-dev 3.11.3`。

### 实测 vs 基线：偏差表

| 项 | 计划基线 | 实测 | 说明 |
|---|---|---|---|
| 系统 | Ubuntu 22.04（kernel 5.15） | **Ubuntu 24.04.3（kernel 7.0.0-28）** | 可接受；内核更新，Landlock / cgroup v2 支持更完整 |
| 编译器 | g++ 11.4（glibc 2.35） | **g++ 13.3.0** | 可接受；白名单靠 strace 实测，不绑定版本。**需记入计划「零点五」** |
| Redis | 7.x | **7.0.15** | 符合 |
| MySQL | 8.x | **8.0.46** | 符合 |
...
`E9` 在 `sudo` 路径下 `exit=0`，且 LSM 列表含 `apparmor`——印证第 7 节判断：**Ubuntu 24.04 的非特权 userns 限制不影响 root 路径**，当前无需 `sysctl` 放宽。若后续沙箱改用 rootless，再回来处理。

**12/12 全绿** → 已具备进入《第三版执行计划》Wave 0 的 8 条隔离冒烟测试的条件。

---

## 附：收尾回填

1. ✅ 本清单结果已回填《环境体检.md》第七节 7.5/7.6；
2. ⏳ 版本偏差（24.04 / g++ 13 vs 22.04 / g++ 11）记入《第三版执行计划.md》「零点五」——**待办**；
3. ✅ 按 AGENTS.md 建 commit。

> **验证声明**：本清单 E1~E12 均为 **VM 内实机执行输出**，逐项核销。版本号（g++ 13.3.0 / cmake 3.28.3 / redis 7.0.15 / mysql 8.0.46 / strace 6.8）为**实测值**，不再是预测。
